...
in
{
  options.services.securo = {
    ...
    credentials = mkOption {
      ...
      default = { };
      example = {
        SECRET_KEY = "/run/secrets/securo_secret_key";
      };
      description = ''
        Credentials injected into all Securo systemd services via {manpage}`systemd.exec(5)`'s
        `LoadCredential` mechanism. Each attribute name is the credential name (exposed as
        a file in `$CREDENTIALS_DIRECTORY`), and the value is the path to the credential
        file on disk.
      '';
    };
